...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      // http://cse.csusb.edu/tongyu/courses/cs660/labs/lab4.php
...
lcg_rand(unsigned long a){
	...
	return (a * 33335264UL) % 538145624UL;
}
  ...
  int chosen;
	...
	for(;;){
		...
		if(total_tickets > 0){
        	...
        	//	cprintf("Process chosen : #%s \n", p->name);
        		
        	c->proc = p;
     	 	switchuvm(p);
      		p->state = RUNNING;
			swtch(&(c->lotteryScheduler), p->context);
      		switchkvm();
		    c->proc = 0;
			}	
...
sched(void)
{
 ...
 // swtch(&p->context, mycpu()->scheduler);
  swtch(&p->context, mycpu()->lotteryScheduler);
